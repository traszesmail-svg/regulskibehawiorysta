...
test('Mapa zachowania speaks to the owner instead of exposing internal funnel language', () => {
  ...
  assert.match(mapSource, /Zanim zaczniemy/)
  assert.match(mapSource, /Zbierzmy fakty\.<br \/>Potem wybierzmy krok\./)
  assert.match(mapSource, /Odpowiedz na dwa krótkie pytania/)
  assert.match(mapSource, /Zapytaj behawiorystę/)
  assert.match(mapSource, /79 zł/)
  assert.match(mapSource, /104 zł/)
  assert.match(mapSource, /Wybierz sposób rozmowy/)
  assert.match(mapSource, /112/)
  assert.match(mapSource, /weterynarzem lub kliniką całodobową/)
  assert.match(mapSource, /Zapisz tę Mapę w swoim Pokoju/)
  assert.match(mapSource, /data-map-screen="safety"/)
  assert.match(mapSource, /data-map-screen="result"/)
  ...
  assert.doesNotMatch(mapSource, /Uporządkuj sytuację psa lub kota krok po kroku/)
  assert.doesNotMatch(mapSource, /Przygotowanie do rozmowy/)
  assert.doesNotMatch(mapSource, /Twoje odpowiedzi pozwolą nam od razu skupić się na sytuacji/)
  assert.doesNotMatch(mapSource, /Twoje odpowiedzi dołączymy do rezerwacji/)
